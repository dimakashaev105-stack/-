...
def init_db():
    ...
    try:
        ...
        required_columns = [
            ('nickname', 'TEXT'),
            ('mining_trees', 'INTEGER DEFAULT 0'),  # Статистика добытых елок
            ('mining_balance', 'INTEGER DEFAULT 0'),  # Текущие накопленные елки
            ('last_snow_work', 'TIMESTAMP'),
            ('snow_cooldown_end', 'TIMESTAMP'),
            ('current_snow_job', 'TEXT'),
            ('snow_job_progress', 'INTEGER DEFAULT 0'),
        ...
        if hours_passed >= 1:
            ...
            if interest > 0:
                ...
                try:
                    ...
                    logging.info(f"Пользователю {user_id} начислены проценты: {interest}❄️")
...
# === СИСТЕМА МАЙНИНГА ЕЛОК ===
MINING_EXCHANGE_RATE = 200  # 1 елка = 1000 снежков
...
def handle_mining(message):
    try:
        ...
        if 'mining_balance' not in columns:
            ...
            conn.commit()
            logging.info(f"Добавлена колонка mining_balance для пользователя {user_id}")
        ...
        accumulated_trees = 0
        if video_cards > 0 and last_collect:
            try:
                if isinstance(last_collect, str):
                    try:
                        last_collect_time = datetime.strptime(last_collect, '%Y-%m-%d %H:%M:%S').timestamp()
                    except:
                        last_collect_time = time.time() - 86400
                else:
                    last_collect_time = last_collect if last_collect else time.time() - 86400
                
                current_time = time.time()
                time_passed = current_time - last_collect_time
                
                if time_passed > 0:
                    # 1 уровень = 1 елка в час, 2 уровень = 2 елки, и т.д.
    ...
    try:
        if call.data == "mining_collect":
                ...
                try:
                    if isinstance(last_collect, str):
                        try:
                            ...
                            last_collect_time = current_time - 86400
                    else:
                        last_collect_time = last_collect if last_collect else current_time - 86400
                    ...
                    logging.error(f"Ошибка расчета накоплений в колбэке: {e}")
                    accumulated_trees = 0
            ...
            if accumulated_trees > 0:
                ...
                cursor.execute(
                    'UPDATE users SET mining_balance = ?, last_mining_collect = ?, mining_trees = COALESCE(mining_trees, 0) + ? WHERE user_id = ?',
                ...
                try:
                    bot.edit_message_text(
                        ...
                        reply_markup=create_mining_keyboard(new_video_cards, 0, new_mining_balance, new_card_price),
                        parse_mode='Markdown'
                    )
                except Exception as e:
            ...
            if balance >= card_price:
                ...
                cursor.execute('SELECT mining_balance FROM users WHERE user_id = ?', (user_id,))
            ...
            cursor.execute('SELECT video_cards FROM users WHERE user_id = ?', (user_id,))
...
💰 *Общий баланс:* ❄️{format_balance(total_balance)}
🏦 *Всего на вкладах:* ❄️{format_balance(total_deposit)}
...
def handle_me(message):
    try:
        ...
        cursor.execute('''
            SELECT first_name, video_cards, bank_deposit, registered_at, mining_balance, nickname 
        ...
        first_name, video_cards, bank_deposit, registered_at, mining_balance, nickname = result
        
        # Используем никнейм если есть, иначе имя
        display_name = nickname.strip() if nickname and nickname.strip() else first_name
        ...
        message_text += f"🎄 *Елки:* 🎄{mining_balance if mining_balance else 0}\n"
        message_text += f"📅 *Регистрация:* {reg_date}"
        
        
        
        # Добавляем информацию о вкладе если есть
        if bank_deposit and bank_deposit > 0:
            message_text += f"\n🏦 *В банке:* ❄️{format_balance(bank_deposit)}"
        ...
        try:
            ...
            if os.path.exists(snowman_path):
                ...
                if current_house:
                    ...
                    if house_image and os.path.exists(house_image):
                        try:
                            ...
                            if house_img.size != snowman_img.size:
                                snowman_img = snowman_img.resize(house_img.size, Image.Resampling.LANCZOS)
                            
                            combined = Image.new('RGBA', house_img.size)
                            ...
                            logging.error(f"Ошибка объединения изображений: {e}")
                            # Продолжаем с обычным фото
