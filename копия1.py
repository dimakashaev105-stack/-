...
def handle_daily_bonus(message):
    try:
        ...
        # Проверяем наличие колонки last_bonus
    ...
    try:
        ...
        try:
            ...
            if channel_member.status in ['member', 'administrator', 'creator']:
                ...
                # Создаем фейковое сообщение для вызова обработчика бонуса
                class FakeMessage:
                    def __init__(self):
                        self.chat = type('obj', (object,), {'id': call.message.chat.id})()
                        self.from_user = type('obj', (object,), {'id': user_id})()
        ...
        logging.error(f"Ошибка в проверке подписки: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")
    ...
    try:
        ...
        if result and result[0]:
            ...
            if last_bonus_time:
                ...
                if time_passed < 1700:  # 28 минут (немного меньше для надежности)
