...
def handle_contest_steps(message):
    ...
    try:
        user_id = message.from_user.id
        contest_data = USER_CONTESTS[user_id]
        ...
        if step == 1:
            ...
            try:
                ...
                max_participants = contest_data["data"]["max_participants"]
...
def create_contest_post(contest_data, contest_id):
    ...
    participate_link = f"https://t.me/{(bot.get_me()).username}?start=contest_{contest_id}"
...
def handle_contest_participation(message):
    ...
    try:
        ...
        contest_id = message.text.split()[1].split('_')[1]
...
def handle_contest_results(message):
    ...
    try:
        ...
        for winner_id in winners:
            try:
                bot.send_message(
                    ...
                    f"Конкурс: {contest.get('channel_title', 'N/A')}\n"
                    f"Место: {winners.index(winner_id) + 1}\n\n"
                    f"Свяжитесь с организатором @{contest.get('creator_username', 'N/A')}",
                    parse_mode='Markdown'
                )
                notified += 1
            except:
        ...
        if contest_id in CONTEST_PARTICIPANTS:
...
def handle_contest_participants(message):
    ...
    try:
        ...
        participants_text += f"\n\n📊 Нужно: {contest['max_participants']}"
