...
# === МИНИМАЛИСТИЧНЫЙ РЫНОК ID (ТОЛЬКО КОМАНДЫ) ===
...
def get_id_owner(id_number):
    """Проверяет, есть ли владелец у ID"""
    return get_user_by_id_number(id_number)

def transfer_id(id_number, from_user_id, to_user_id):
    """Передает ID от одного пользователя другому"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Получаем всех пользователей по порядку регистрации
        cursor.execute('''
        SELECT user_id FROM users 
        WHERE is_banned = 0 
        ORDER BY registered_at ASC
        ...
        all_users = cursor.fetchall()
        users_list = [u[0] for u in all_users]
        
        # Находим индекс текущего владельца
        if from_user_id in users_list:
            current_index = users_list.index(from_user_id)
        else:
            conn.close()
            return False, "Владелец не найден"
        
        # Меняем местами
        users_list[current_index] = to_user_id
        
        # Пересоздаем таблицу с новым порядком (это сложно, проще отметить в отдельной таблице)
        # Но для простоты будем использовать отдельную таблицу владения ID
        
        conn.close()
        
        # Используем отдельную таблицу для хранения владельцев ID
        conn = get_db_connection()
        cursor = conn.cursor()
        ...
        if existing:
            # Обновляем владельца
            cursor.execute('''
            UPDATE id_ownership SET owner_id = ?, purchased_at = CURRENT_TIMESTAMP
            WHERE id_number = ?
            ''', (to_user_id, id_number))
        else:
            # Создаем новую запись
            cursor.execute('''
            INSERT INTO id_ownership (id_number, owner_id)
            VALUES (?, ?)
            ''', (id_number, to_user_id))
        
        conn.commit()
        conn.close()
        
        return True, "ID успешно передан"
        
    except Exception as e:
        logging.error(f"Ошибка передачи ID: {e}")
        return False, str(e)

def get_current_id_owner(id_number):
    """Получает текущего владельца ID"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
...
def format_id_info(id_number):
    """Форматирует информацию о ID"""
    owner_id = get_current_id_owner(id_number)
...
def handle_id_sell(message):
    ...
    try:
        ...
        # Получаем номер ID пользователя
        user_id_number = get_user_id_number(user_id)
        
        # Проверяем, не продан ли уже этот ID
        current_owner = get_current_id_owner(user_id_number)
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Создаем таблицу продаж если её нет
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS id_sales (
            sale_id INTEGER PRIMARY KEY AUTOINCREMENT,
            seller_id INTEGER NOT NULL,
        ...
        # Проверяем, нет ли уже активной продажи этого ID
        cursor.execute('''
        ...
        if existing:
            bot.send_message(message.chat.id, 
                           "❌ Этот ID уже выставлен на продажу!\n"
                           "Сначала отмени старую продажу командой `идоtmена`",
                           parse_mode='Markdown')
            conn.close()
            return
        ...
        bot.send_message(
            ...
            f"{format_id_info(user_id_number)}\n"
            f"💰 Цена: {format_balance(price)}🌸\n\n"
            f"💡 Купить: `идкуп {user_id_number}`",
            parse_mode='Markdown'
        )
...
def handle_id_buy(message):
    ...
    try:
        ...
        # Проверяем, не свой ли ID покупает
        ...
        if not sale:
            bot.send_message(
                message.chat.id, 
                f"❌ ID #{target_id} сейчас не продается\n"
        ...
        try:
            ...
            # Помечаем продажу как завершенную
            cursor.execute('''
            UPDATE id_sales SET status = 'sold' WHERE sale_id = ?
            ''', (sale_id,))
            
            # Создаем таблицу истории если её нет
            cursor.execute('''
            CREATE TABLE IF NOT EXISTS id_history (
                history_id INTEGER PRIMARY KEY AUTOINCREMENT,
                seller_id INTEGER NOT NULL,
            ...
            # Передаем ID новому владельцу
            success, msg = transfer_id(target_id, seller_id, user_id)
...
def handle_id_offer(message):
    ...
    try:
        ...
        user_own_id = get_user_id_number(user_id)
        if user_own_id == target_id:
            bot.send_message(message.chat.id, "❌ Нельзя предлагать цену за свой ID")
            return
        
        # Получаем владельца ID
        owner_id = get_current_id_owner(target_id)
        
        if not owner_id:
            bot.send_message(
                message.chat.id,
                f"❌ ID #{target_id} никому не принадлежит\n"
                f"💡 Его можно купить по минимальной цене"
            )
            return
        
        if owner_id == user_id:
            bot.send_message(message.chat.id, "❌ Это твой собственный ID")
            return
        ...
        # Создаем таблицу офферов если её нет
        cursor.execute('''
        CREATE TABLE IF NOT EXISTS id_offers (
            offer_id INTEGER PRIMARY KEY AUTOINCREMENT,
            buyer_id INTEGER NOT NULL,
        ...
        if existing:
            bot.send_message(
                message.chat.id,
                "❌ У тебя уже есть активное предложение по этому ID"
            )
            conn.close()
            return
        ...
        buyer_id_number = get_user_id_number(user_id)
        ...
        try:
            bot.send_message(
                ...
                f"💡 Средства покупателя заморожены\n\n"
...
def handle_id_accept(message):
    ...
    try:
        ...
        if len(parts) < 2:
            bot.send_message(message.chat.id, "❌ Формат: `идпринять (номер предложения)`")
            return
        
        try:
            offer_id = int(parts[1])
        except ValueError:
            bot.send_message(message.chat.id, "❌ Неверный номер предложения")
            return
        ...
        # Проверяем, что принимает именно владелец
        if owner_id != user_id:
        ...
        try:
            # Деньги покупателя уже списаны, просто начисляем продавцу
            cursor.execute('''
            UPDATE users SET balance = balance + ? WHERE user_id = ?
            ''', (price, user_id))
            ...
            cursor.execute('''
            UPDATE id_offers SET status = 'accepted' WHERE offer_id = ?
            ''', (offer_id,))
            
            # Передаем ID новому владельцу
            success, msg = transfer_id(id_number, user_id, buyer_id)
            ...
            # Получаем новый баланс продавца
            cursor.execute('SELECT balance FROM users WHERE user_id = ?', (user_id,))
            new_balance = cursor.fetchone()[0]
            ...
            cursor.execute('ROLLBACK')
            logging.error(f"Ошибка транзакции: {e}")
            bot.send_message(message.chat.id, "❌ Ошибка при принятии предложения")
...
def handle_id_reject(message):
    ...
    try:
        ...
        if len(parts) < 2:
            bot.send_message(message.chat.id, "❌ Формат: `идотклонить (номер предложения)`")
            return
        ...
        # Проверяем, что отклоняет именно владелец
        if owner_id != user_id:
            bot.send_message(message.chat.id, "❌ Это не твое предложение")
            conn.close()
            return
        ...
        try:
            ...
            # Обновляем статус предложения
            cursor.execute('''
            UPDATE id_offers SET status = 'rejected' WHERE offer_id = ?
            ''', (offer_id,))
            ...
            cursor.execute('ROLLBACK')
            logging.error(f"Ошибка транзакции: {e}")
            bot.send_message(message.chat.id, "❌ Ошибка при отклонении")
...
def handle_id_cancel_sale(message):
    ...
    try:
        ...
        # Отменяем продажу
        cursor.execute('''
        UPDATE id_sales SET status = 'cancelled' WHERE sale_id = ?
        ''', (sale[0],))
        ...
        bot.send_message(
            message.chat.id,
            f"✅ Продажа ID #{user_id_number} отменена",
            parse_mode='Markdown'
        )
...
@bot.message_handler(func=lambda message: message.text.lower().startswith('ид '))
def handle_id_info(message):
    """Проверить информацию об ID"""
    try:
        if is_spam(message.from_user.id):
            return
        ...
        try:
            ...
            bot.send_message(
                message.chat.id,
                format_id_info(target_id),
                parse_mode='Markdown'
            )
        except ValueError:
            bot.send_message(message.chat.id, "❌ Неверный номер ID")
        
    except Exception as e:
        logging.error(f"Ошибка проверки ID: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка")

# === КОМАНДА СПИСКА ПРОДАЖ ===

@bot.message_handler(func=lambda message: message.text.lower() == 'идрынок')
def handle_id_market_list(message):
    ...
    try:
        ...
        for id_number, price, seller_id in sales:
            seller_number = get_user_id_number(seller_id)
            text += f"🎯 #{id_number} — {format_balance(price)}🌸 (от #{seller_number})\n"
        ...
        logging.error(f"Ошибка списка продаж: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка")

# === КОМАНДА МОИ ПРЕДЛОЖЕНИЯ ===

@bot.message_handler(func=lambda message: message.text.lower() == 'идмои')
def handle_id_my_offers(message):
    ...
    try:
        ...
        # Мои исходящие предложения (я покупатель)
        cursor.execute('''
        ...
        # Мои входящие предложения (я владелец)
        cursor.execute('''
        SELECT offer_id, id_number, price, status, buyer_id 
        FROM id_offers 
        ...
        if incoming:
            text += "📨 ВХОДЯЩИЕ:\n"
            for offer_id, id_number, price, status, buyer_id in incoming[:3]:
                buyer_number = get_user_id_number(buyer_id)
                text += f"• #{offer_id}: ID{id_number} — {format_balance(price)}🌸 от #{buyer_number}\n"
                text += f"  ✅ `идпринять {offer_id}` ❌ `идотклонить {offer_id}`\n"
            text += "\n"
        
        if outgoing:
            text += "📤 ИСХОДЯЩИЕ:\n"
            for offer_id, id_number, price, status, owner_id in outgoing:
                status_emoji = "⏳" if status == "pending" else "✅" if status == "accepted" else "❌"
                owner_number = get_user_id_number(owner_id)
                text += f"{status_emoji} #{offer_id}: ID{id_number} — {format_balance(price)}🌸 владельцу #{owner_number}\n"
        
        if not incoming and not outgoing:
            text += "Нет активных предложений\n\n"
            text += "💡 Предложи цену: `идпред (номер) (сумма)`"
        
        bot.send_message(message.chat.id, text, parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Ошибка списка предложений: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка")
...
def create_main_menu(chat_id):
    ...
    if chat_id > 0:
        markup.add(
            ...
            KeyboardButton("🏠 Дом"),
            KeyboardButton("Бонус"),
            KeyboardButton("🏪 Рынок ID")  # Кнопка для быстрого доступа
        )
    else:
        markup.add(
