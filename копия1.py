...
def init_db():
    ...
    try:
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_id_offers_target ON id_offers(target_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_id_history_sold ON id_history(sold_at)')
        
        conn.commit()
        logging.info("✅ База данных успешно инициализирована со всеми таблицами, включая рынок ID")
