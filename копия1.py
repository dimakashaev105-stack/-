...
# =============== ИСПРАВЛЕННАЯ СИСТЕМА КОНКУРСОВ ===============
...
CONTEST_ADMINS = {}  # {channel_id: [admin_ids]} для кэша
...
def handle_contest_start(message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            "🎉 *СОЗДАНИЕ КОНКУРСА*\n\n"
            "1️⃣ *Шаг 1:* Пришлите ссылку на канал\n"
        ...
        bot.send_message(message.chat.id, "❌ Ошибка создания конкурса")
...
def handle_contest_steps(message):
    ...
    try:
        ...
        if not contest_data:
            bot.send_message(message.chat.id, "❌ Данные конкурса не найдены")
            return
        ...
        if step == 1:
            ...
            if not channel_username:
                bot.send_message(message.chat.id, 
                               ...
                               "Используйте: @channel или https://t.me/channel")
                return
            
            # Проверяем права в канале
            bot.send_message(message.chat.id, "⏳ Проверяю права в канале...")
            
            channel_info = check_channel_admin(user_id, channel_username)
            if not channel_info:
                bot.send_message(message.chat.id,
                               "❌ Ошибка проверки прав!\n\n"
                               "Убедитесь что:\n"
                               "1. Вы администратор канала\n"
                               "2. Бот администратор канала\n"
                               "3. Канал публичный")
                return
            
            # Сохраняем данные канала
            contest_data["data"]["channel_id"] = channel_info["id"]
            contest_data["data"]["channel_username"] = channel_username
            contest_data["data"]["channel_title"] = channel_info["title"]
            contest_data["step"] = 2
            
            bot.send_message(
                message.chat.id,
                f"✅ Канал: {channel_info['title']}\n\n"
                f"2️⃣ *Шаг 2:* Введите количество участников\n"
                f"Минимум: 2\n"
                f"Максимум: 1000\n\n"
                f"Пример: `50`",
                parse_mode='Markdown'
            ...
            try:
                ...
                if max_participants < 2:
                    bot.send_message(message.chat.id, "❌ Минимум 2 участника")
                    return
                if max_participants > 1000:
                    bot.send_message(message.chat.id, "❌ Максимум 1000 участников")
                    return
                    
                contest_data["data"]["max_participants"] = max_participants
                contest_data["step"] = 3
                
                bot.send_message(
                    message.chat.id,
                    f"✅ Участников: {max_participants}\n\n"
                    f"3️⃣ *Шаг 3:* Введите количество победителей\n"
            ...
            try:
                ...
                max_participants = contest_data["data"].get("max_participants", 10)
                ...
                contest_data["data"]["winners_count"] = winners_count
                contest_data["step"] = 4
                
                bot.send_message(
                    message.chat.id,
                    f"✅ Победителей: {winners_count}\n\n"
                    f"4️⃣ *Шаг 4:* Введите призы для победителей\n"
                    f"Формат: место → приз\n\n"
                    f"*Пример:*\n"
                    f"```\n"
            ...
            if not prizes_text:
                bot.send_message(message.chat.id, "❌ Введите текст призов")
                return
                
            contest_data["data"]["prizes_text"] = prizes_text
            contest_data["step"] = 5
            
            # Формируем итоговый текст
            final_data = contest_data["data"]
            
            preview_text = create_contest_preview(final_data)
        ...
        logging.error(f"Ошибка в шаге конкурса: {e}")
...
def extract_channel_username(input_text):
    """Извлекает username канала из текста"""
    try:
        input_text = input_text.strip()
        
        # Убираем https://t.me/
        if "t.me/" in input_text:
            input_text = input_text.split("t.me/")[-1]
        ...
        input_text = input_text.split()[0].split('?')[0]
        
        return input_text if input_text and len(input_text) > 1 else None
    except:
        return None

def check_channel_admin(user_id, channel_username):
    """Проверяет права администратора в канале"""
    try:
        # Пробуем получить информацию о канале
        try:
            chat = bot.get_chat(f"@{channel_username}")
        except Exception as e:
            logging.error(f"Не могу получить канал @{channel_username}: {e}")
            return None
        
        # Проверяем что это канал
        if chat.type != 'channel':
            logging.error(f"@{channel_username} не канал, тип: {chat.type}")
            return None
        ...
        try:
            ...
            logging.error(f"Не могу получить админов канала: {e}")
            return None
        ...
        if user_id not in admin_ids:
            logging.error(f"Пользователь {user_id} не админ канала")
            return None
        
        # Проверяем что бот администратор
        bot_id = bot.get_me().id
        if bot_id not in admin_ids:
            logging.error(f"Бот не админ канала")
            return None
        
        # Кэшируем администраторов
        CONTEST_ADMINS[chat.id] = admin_ids
...
def create_contest_preview(contest_data):
    """Создает текст для предпросмотра конкурса"""
...
def create_contest_post(contest_data, contest_id):
    """Создает пост конкурса для публикации"""
    ...
    participate_link = f"https://t.me/{bot_username}?start=contest_{contest_id}"
    
    text = f"""
🎊 *КОНКУРС!* 🎊

*📢 Канал:* @{contest_data.get('channel_username', 'N/A')}
*👥 Нужно участников:* {contest_data.get('max_participants', 'N/A')}
*🏆 Победителей:* {contest_data.get('winners_count', 'N/A')}
...
2. Вы автоматически подпишетесь
3. Ждите результатов!

*⏰ Участники набираются автоматически*
    ...
    try:
        ...
        if call.data == "contest_publish":
            if user_id not in USER_CONTESTS:
                bot.answer_callback_query(call.id, "❌ Конкурс не найден")
                return
            
            contest_data = USER_CONTESTS[user_id]["data"]
            
            # Проверяем необходимые данные
            required_fields = ['channel_id', 'channel_username', 'channel_title', 
                             'max_participants', 'winners_count', 'prizes_text']
            
            for field in required_fields:
                if field not in contest_data:
                    bot.answer_callback_query(call.id, f"❌ Отсутствует поле: {field}")
                    return
            
            # Генерируем ID конкурса
            contest_id = f"{user_id}_{int(time.time())}"
            
            # Создаем пост
            post_text, markup = create_contest_post(contest_data, contest_id)
            
            # Публикуем в канале
            try:
                # Проверяем возможность отправки в канал
                bot.send_chat_action(contest_data["channel_id"], 'typing')
                ...
                try:
                    bot.edit_message_text(
                        ...
                        f"📢 Канал: {contest_data['channel_title']}\n"
                        f"👥 Участников: 0/{contest_data['max_participants']}\n"
                        f"🏆 Победителей: {contest_data['winners_count']}\n"
                ...
                if "CHAT_WRITE_FORBIDDEN" in error_msg:
                    bot.answer_callback_query(call.id, "❌ Бот не может писать в канал")
                elif "CHAT_ADMIN_REQUIRED" in error_msg:
                    bot.answer_callback_query(call.id, "❌ Бот не админ канала")
                elif "chat not found" in error_msg.lower():
                    bot.answer_callback_query(call.id, "❌ Канал не найден")
                else:
                    bot.answer_callback_query(call.id, f"❌ Ошибка: {error_msg[:50]}")
                return
            ...
            if user_id in USER_CONTESTS:
                ...
                bot.answer_callback_query(call.id, "❌ Данные утеряны")
            ...
            try:
                ...
                bot.send_message(call.message.chat.id, "❌ Создание конкурса отменено")
...
@bot.message_handler(func=lambda message: message.text and 'contest_' in message.text and message.text.startswith('/start'))
def handle_contest_participation(message):
    ...
    try:
        ...
        contest_param = parts[1]
        if not contest_param.startswith('contest_'):
            return
            
        contest_id = contest_param.split('_')[1]
        
        if not contest_id:
            bot.send_message(message.chat.id, "❌ Неверная ссылка конкурса")
            return
        ...
        channel_username = contest.get('channel_username')
        if channel_username:
            try:
                chat_member = bot.get_chat_member(f"@{channel_username}", user_id)
                if chat_member.status not in ['member', 'administrator', 'creator']:
                    markup = InlineKeyboardMarkup()
                    markup.add(InlineKeyboardButton("📢 ПОДПИСАТЬСЯ", url=f"https://t.me/{channel_username}"))
                    
                    bot.send_message(
                        message.chat.id,
                        f"❌ Для участия подпишитесь на канал\n"
                        f"📢 @{channel_username}",
                        reply_markup=markup
                    )
                    return
                ...
                # Продолжаем без проверки, если ошибка
        
        # Проверяем уже ли участвует
        if user_id in CONTEST_PARTICIPANTS.get(contest_id, []):
            bot.send_message(message.chat.id, "✅ Вы уже участвуете в конкурсе!")
            return
        
        # Проверяем лимит участников
        current_count = len(CONTEST_PARTICIPANTS.get(contest_id, []))
        max_participants = contest.get('max_participants', 0)
        
        if current_count >= max_participants:
            bot.send_message(message.chat.id, "❌ Конкурс уже набрал максимальное количество участников")
            return
        ...
        # Обновляем счетчик
        current_count = len(CONTEST_PARTICIPANTS[contest_id])
        ...
        bot.send_message(
            ...
            f"👥 Участников: {current_count}/{max_participants}\n"
            f"🏆 Победителей: {contest.get('winners_count', 'N/A')}\n\n"
            f"*Ждите результатов!*",
            parse_mode='Markdown'
        )
        
        # Уведомляем организатора если нужно
        if current_count == max_participants:
            try:
                bot.send_message(
                    contest['creator_id'],
                ...
                logging.error(f"Не удалось уведомить организатора: {e}")
        
        # Логируем участие
        log_user_action(user_id, "CONTEST_JOIN", f"contest={contest_id}")
        
    except Exception as e:
        logging.error(f"Ошибка участия в конкурсе: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка участия в конкурсе")

# =============== КОМАНДЫ УПРАВЛЕНИЯ КОНКУРСАМИ ===============
...
def handle_contest_results(message):
    ...
    try:
        ...
        try:
            ...
            bot.send_message(message.chat.id, "❌ Ошибка выбора победителей")
            return
        ...
        try:
            ...
            bot.send_message(message.chat.id, f"❌ Не удалось опубликовать в канале: {e}")
        
        # Уведомляем победителей
        notified = 0
        for i, winner_id in enumerate(winners, 1):
            try:
                creator_username = contest.get('creator_username', '')
                if not creator_username and contest.get('creator_id'):
                    try:
                        creator_chat = bot.get_chat(contest['creator_id'])
                        creator_username = creator_chat.username if creator_chat.username else ""
                    except:
                        creator_username = ""
                
                contact = f"@{creator_username}" if creator_username else "организатору"
                ...
                bot.send_message(
                    ...
                    f"Конкурс: {contest.get('channel_title', 'N/A')}\n"
                    f"Место: {i}\n\n"
                    f"Свяжитесь с {contact} для получения приза",
                    parse_mode='Markdown'
                )
                notified += 1
            except Exception as e:
        ...
        bot.send_message(
            ...
            f"👤 Уведомлено победителей: {notified}/{len(winners)}"
        )
...
def handle_contest_participants(message):
    ...
    try:
        ...
        # Формируем список
        participants_text = f"👥 *УЧАСТНИКИ ({len(participants)}):*\n\n"
        
        for i, user_id in enumerate(participants[:50], 1):  # Ограничиваем 50
            try:
                user = bot.get_chat(user_id)
                username = f"@{user.username}" if user.username else user.first_name
                participants_text += f"{i}. {username}\n"
            except:
                participants_text += f"{i}. ID: {user_id}\n"
        
        if len(participants) > 50:
            participants_text += f"\n... и ещё {len(participants) - 50}"
        
        participants_text += f"\n\n📊 Нужно: {contest.get('max_participants', 0)}"
        
        bot.send_message(message.chat.id, participants_text, parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Ошибка показа участников: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка получения участников")

@bot.message_handler(func=lambda message: message.text.lower().startswith('моиконкурсы') and is_admin(message.from_user.id))
def handle_my_contests(message):
    """Показывает созданные конкурсы"""
    try:
        user_id = message.from_user.id
        ...
        contests_text = "🎯 *ВАШИ КОНКУРСЫ:*\n\n"
        
        for contest_id, contest, participants in my_contests:
            contests_text += (
                f"*{contest.get('channel_title', 'N/A')}*\n"
                f"ID: `{contest_id}`\n"
                f"Участников: {participants}/{contest.get('max_participants', 'N/A')}\n"
                f"Статус: {'✅ Активен' if contest.get('status') == 'active' else '❌ Завершен'}\n\n"
            )
        
        contests_text += "*Команды:*\n`итоги ID` `участники ID` `отмена ID`"
        
        bot.send_message(message.chat.id, contests_text, parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Ошибка показа конкурсов: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка получения конкурсов")
...
def handle_contest_cancel(message):
    ...
    try:
        ...
        try:
            ...
            logging.error(f"Не удалось уведомить канал: {e}")
        ...
        logging.error(f"Ошибка отмены конкурса: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка отмены конкурса")

# =============== КОМАНДА ДЛЯ ВЫВОДА ВСЕХ КОНКУРСОВ ===============

@bot.message_handler(func=lambda message: message.text.lower() == 'всеконкурсы' and is_admin(message.from_user.id))
def handle_all_contests(message):
    """Показывает все активные конкурсы"""
    try:
        if not ACTIVE_CONTESTS:
            bot.send_message(message.chat.id, "📭 Нет активных конкурсов")
            return
        
        contests_text = "🎯 *ВСЕ АКТИВНЫЕ КОНКУРСЫ:*\n\n"
        
        for i, (contest_id, contest) in enumerate(ACTIVE_CONTESTS.items(), 1):
            participants = len(CONTEST_PARTICIPANTS.get(contest_id, []))
            contests_text += (
                f"{i}. *{contest.get('channel_title', 'N/A')}*\n"
                f"   ID: `{contest_id}`\n"
                f"   Участников: {participants}/{contest.get('max_participants', 'N/A')}\n"
                f"   Создатель: {contest.get('creator_name', 'N/A')}\n\n"
            )
        
        bot.send_message(message.chat.id, contests_text, parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Ошибка показа всех конкурсов: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка получения конкурсов")

# =============== АВТООЧИСТКА СТАРЫХ КОНКУРСОВ ===============
...
def cleanup_old_contests():
    ...
    while True:
        ...
        to_remove = []
        for contest_id, contest in ACTIVE_CONTESTS.items():
            # Удаляем конкурсы старше 30 дней
            if current_time - contest.get('created_at', current_time) > 2592000:
...
`всеконкурсы` — Все активные конкурсы

*Команды управления:*
`итоги ID` — Подвести итоги
`участники ID` — Участники конкурса
`отмена ID` — Отменить конкурс

*Как участвовать:*
1. Найдите пост конкурса в канале
2. Нажмите кнопку "УЧАСТВОВАТЬ"
3. Подпишитесь на канал если нужно
4. Ждите результатов!

*Требования:*
• Организатор должен быть админом канала
