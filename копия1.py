...
def cleanup_contests():
    while True:
        ...
        for contest_id, contest in ACTIVE_CONTESTS.items():
            if current - contest.get('created_at', current) > 604800:  # 7 дней
...
# Запуск очистки
import threading
cleanup_thread = threading.Thread(target=cleanup_contests, daemon=True)
