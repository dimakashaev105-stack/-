...
def handle_contest_participation(message):
    ...
    try:
        ...
        if user_id in CONTEST_PARTICIPANTS.get(contest_id, []):
            bot.send_message(message.chat.id, "✅ Вы уже участвуете!")
            return
        ...
        logging.error(f"Ошибка участия: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка участия")
...
👤 Организатор: {contest.get('creator_name', 'N/A')}"""
        ...
        bot.send_message(
            ...
            f"👤 Уведомлено: {notified}/{len(winners)}"
        )
        ...
        if result:
            ...
            if current_house:
                ...
                if house_image and os.path.exists(house_image):
                    img_data = create_house_image(house_image, user_id)
                    
                    if img_data:
                        try:
                            # Отправляем фото дома с подписью
                            bot.send_photo(message.chat.id, img_data, caption=message_text)
                            return
                        except Exception as e:
                            logging.error(f"Ошибка при отправке фото дома: {e}")
                            # Если не удалось, пробуем старую картинку
                            pass
...
# === МАГАЗИН ДОМОВ (ИНЛАЙН С ПАГИНАЦИЕЙ) ===
def create_house_shop_keyboard(page=1, items_per_page=3):
    ...
    end_idx = min(start_idx + items_per_page, total_houses)
    
    # Добавляем кнопки для домов
    for i in range(start_idx, end_idx):
        house_id = house_ids[i]
        house_info = HOUSE_SHOP[house_id]
        
        button_text = f"🏡 {house_info['name']} - {format_balance(house_info['price'])}❄️"
        markup.add(InlineKeyboardButton(button_text, callback_data=f"buy_house_{house_id}"))
...
def create_house_shop_message(page=1):
    """Создает сообщение для магазина домов"""
    house_ids = list(HOUSE_SHOP.keys())
    total_houses = len(house_ids)
    
    if total_houses == 0:
        return "🛒 *Магазин домов*\n\n🚫 В магазине пока нет домов.\n\nАдминистратор может добавить дома командой:\n`дом [цена] [название_файла.png]`"
    ...
    message = f"🛒 *Магазин домов* • Страница {page}/{total_pages}\n\n"
    
    for i in range(start_idx, end_idx):
        house_id = house_ids[i]
        house_info = HOUSE_SHOP[house_id]
        
        message += f"🏡 *{house_info['name']}*\n"
        message += f"💰 Цена: {format_balance(house_info['price'])}❄️\n"
        message += f"🆔 `{house_id}`\n\n"
    
    message += "💡 Нажмите на дом чтобы купить его"
    return message

# === ШКАФ (ИНВЕНТАРЬ ДОМОВ) ===
def create_wardrobe_keyboard(user_id, page=1, items_per_page=3):
    ...
    # Добавляем кнопки для домов
    for i in range(start_idx, end_idx):
        house_id, is_current = houses[i]
        house_info = HOUSE_SHOP.get(house_id, {"name": "Неизвестный дом"})
        
        if is_current or house_id == current_house:
            button_text = f"✅ {house_info['name']}"
            callback_data = f"wardrobe_current_{house_id}"
        else:
            button_text = f"🏡 {house_info['name']}"
            callback_data = f"set_house_{house_id}"
...
def create_wardrobe_message(user_id, page=1):
    """Создает сообщение для шкафа"""
    houses = get_user_houses(user_id)
    total_houses = len(houses)
    ...
    message = f"🚪 *Ваш шкаф* • Страница {page}/{total_pages}\n\n"
    ...
    for i in range(start_idx, end_idx):
        ...
        if is_current or house_id == current_house:
            message += f"✅ *{house_info['name']}* (текущий)\n"
        else:
            message += f"🏡 *{house_info['name']}*\n"
        
        message += f"🆔 `{house_id}`\n\n"
    
    message += "💡 Нажмите на дом чтобы выбрать его как текущий"
    return message

# === ФУНКЦИЯ ДЛЯ СОЗДАНИЯ ИЗОБРАЖЕНИЯ ДОМА С g.png ===
def create_house_image(house_filename, user_id=None):
    """Создает изображение дома с наложением g.png (просто накладывает)"""
    try:
        # Проверяем существование файла дома
        if not os.path.exists(house_filename):
            logging.error(f"Файл дома не найден: {house_filename}")
            return None
        
        # Определяем поддерживаемые форматы
        supported_formats = ['.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp']
        file_ext = os.path.splitext(house_filename)[1].lower()
        
        if file_ext not in supported_formats:
            logging.error(f"Неподдерживаемый формат файла: {file_ext}")
            return None
        
        # Открываем изображение дома
        try:
            house_img = Image.open(house_filename).convert("RGBA")
        except Exception as e:
            logging.error(f"Ошибка открытия файла дома {house_filename}: {e}")
            return None
        
        # Проверяем существование g.png
        if os.path.exists("g.png"):
            try:
                # Открываем g.png с прозрачным фоном
                g_img = Image.open("g.png").convert("RGBA")
                
                # Получаем размеры
                house_width, house_height = house_img.size
                g_width, g_height = g_img.size
                
            
                position = (0, 0)
                
                # Если g.png больше дома, обрезаем его
                if g_width > house_width or g_height > house_height:
                    # Обрезаем g.png под размер дома
                    g_img = g_img.crop((0, 0, min(g_width, house_width), min(g_height, house_height)))
                    logging.info(f"Обрезан g.png под размер дома")
                
                # Накладываем g.png на дом
                # Позиция (0,0) = верхний левый угол
                house_img.paste(g_img, position, g_img)
                
                logging.info(f"Наложен g.png на дом {house_filename} (позиция 0,0)")
                
            except Exception as e:
                logging.error(f"Ошибка наложения g.png: {e}")
                # Продолжаем без g.png
        else:
            logging.warning("g.png не найден, создаем изображение без наложения")
        
        # Создаем байтовый поток
        img_byte_arr = io.BytesIO()
        house_img.save(img_byte_arr, format='PNG')
        img_byte_arr.seek(0)
        
        return img_byte_arr
        
    except Exception as e:
        logging.error(f"Ошибка создания изображения дома: {e}")
        return None

# === ОБРАБОТЧИКИ КОЛБЭКОВ ДЛЯ ДОМОВ ===
@bot.callback_query_handler(func=lambda call: call.data.startswith(('house_', 'shop_', 'wardrobe_', 'buy_house_', 'set_house_')))
def house_callback_handler(call):
    try:
        user_id = call.from_user.id
        ...
        banned, reason = is_banned(user_id)
        if banned:
            bot.answer_callback_query(call.id, f"🚫 Вы забанены!\nПричина: {reason}")
            return
        
        if call.data == "house_shop":
            # Магазин домов
            message_text = create_house_shop_message(1)
            markup = create_house_shop_keyboard(1)
        ...
        elif call.data == "house_wardrobe":
            # Шкаф
            message_text = create_wardrobe_message(user_id, 1)
            markup = create_wardrobe_keyboard(user_id, 1)
            ...
            if current_house:
                house_info = HOUSE_SHOP.get(current_house, {})
                house_name = house_info.get('name', 'Неизвестный дом')
                house_price = house_info.get('price', 0)
                house_image = house_info.get('image')
                
                # Создаем изображение дома
                if house_image and os.path.exists(house_image):
                    img_data = create_house_image(house_image, user_id)
                        ...
                        try:
                            ...
                            bot.send_photo(
                                ...
                                caption=f"🏠 *Текущий дом*\n\n"
                                      f"🏡 *{house_name}*\n"
                                      f"💰 Куплен за: {format_balance(house_price)}❄️\n\n"
            ...
            help_text = (
                ...
                "3. Ваш g.png автоматически накладывается на дом\n\n"
                "*Для администраторов:*\n"
            ...
            # Пагинация в магазине
            try:
                page = int(call.data.split("_")[2])
                message_text = create_house_shop_message(page)
                markup = create_house_shop_keyboard(page)
            ...
            try:
                page = int(call.data.split("_")[2])
                message_text = create_wardrobe_message(user_id, page)
                markup = create_wardrobe_keyboard(user_id, page)
            ...
            if success:
                # Получаем текущую страницу магазина из сообщения
                try:
                    # Пытаемся определить текущую страницу из текста сообщения
                    import re
                    message_text = call.message.text
                    match = re.search(r'Страница (\d+)/(\d+)', message_text)
                    
                    if match:
                        current_page = int(match.group(1))
                        message_text = create_house_shop_message(current_page)
                        markup = create_house_shop_keyboard(current_page)
                        ...
                        # Если не нашли страницу, просто обновляем
                        message_text = create_house_shop_message(1)
                        markup = create_house_shop_keyboard(1)
                    ...
                    bot.send_message(
                        call.message.chat.id,
                        f"✅ Дом '{house_name}' куплен!\n💰 Потрачено: {format_balance(house_price)}❄️"
                    )
                
                bot.answer_callback_query(call.id, f"✅ Куплен дом '{house_name}'!")
                
                # Отправляем изображение дома
                house_image = house_info.get('image')
                if house_image and os.path.exists(house_image):
                    img_data = create_house_image(house_image, user_id)
            ...
            if success:
                # Получаем текущую страницу шкафа из сообщения
                try:
                    # Пытаемся определить текущую страницу из текста сообщения
                    import re
                    message_text = call.message.text
                    ...
                    if match:
                        current_page = int(match.group(1))
                        message_text = create_wardrobe_message(user_id, current_page)
                        markup = create_wardrobe_keyboard(user_id, current_page)
                    ...
                    bot.send_message(
                        call.message.chat.id,
                        f"✅ Дом '{house_info['name']}' установлен как текущий!"
                    )
                
                bot.answer_callback_query(call.id, f"✅ Выбран дом '{house_info['name']}'!")
                
                # Отправляем изображение нового текущего дома
                house_image = house_info.get('image')
                if house_image and os.path.exists(house_image):
                    img_data = create_house_image(house_image, user_id)
                    ...
                    if img_data:
                        try:
                            bot.send_photo(
                                call.message.chat.id,
                                photo=img_data,
                                caption=f"🏠 *Новый текущий дом!*\n\n"
                                      f"*{house_info['name']}*\n\n"
                                      f"💡 Ваш g.png наложен на дом",
                                parse_mode='Markdown'
                            )
                        except:
                            pass
        ...
        elif call.data.startswith("wardrobe_current_"):
            # Показ текущего дома из шкафа
            house_id = call.data[17:]  # Убираем "wardrobe_current_"
            house_info = HOUSE_SHOP.get(house_id, {})
            
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("🔙 Назад", callback_data="house_wardrobe"))
            
            bot.send_message(
                call.message.chat.id,
                f"✅ Это ваш текущий дом!\n\n"
                f"🏡 *{house_info.get('name', 'Неизвестный дом')}*\n"
                f"💰 Куплен за: {format_balance(house_info.get('price', 0))}❄️",
                reply_markup=markup,
                parse_mode='Markdown'
            )
        ...
        elif call.data == "shop_current" or call.data == "wardrobe_current":
...
# Добавьте этот вызов в начало, после init_db()
load_house_shop()

# === КОМАНДА ДЛЯ ПРОСМОТРА МАГАЗИНА АДМИНОМ ===
@bot.message_handler(func=lambda message: message.text.lower() == 'магазин' and is_admin(message.from_user.id))
def handle_shop_list(message):
    """Показывает список домов в магазине"""
    try:
        if not is_admin(message.from_user.id):
            return
        
        if not HOUSE_SHOP:
            bot.send_message(message.chat.id, "🛒 Магазин домов пуст")
            return
        
        text = "🛒 *Магазин домов*\n\n"
        
        for house_id, house_info in HOUSE_SHOP.items():
            text += f"🏡 *{house_info['name']}*\n"
            text += f"💰 Цена: {format_balance(house_info['price'])}❄️\n"
            text += f"🖼 Файл: {house_info['image']}\n"
            text += f"🆔 ID: `{house_id}`\n\n"
        
        text += f"Всего домов: {len(HOUSE_SHOP)}"
        
        bot.send_message(message.chat.id, text, parse_mode='Markdown')
        
    except Exception as e:
        logging.error(f"Ошибка показа магазина: {e}")
        bot.send_message(message.chat.id, f"❌ Ошибка: {str(e)[:100]}")

# === КОМАНДА ДЛЯ УДАЛЕНИЯ ДОМА ИЗ МАГАЗИНА ===
@bot.message_handler(func=lambda message: message.text.lower().startswith('удалитьдом ') and is_admin(message.from_user.id))
def handle_delete_house(message):
    """Удаляет дом из магазина"""
    try:
        if not is_admin(message.from_user.id):
            return
        ...
        parts = message.text.split()
        if len(parts) < 2:
            bot.send_message(message.chat.id, "❌ Формат: удалитьдом [ID_дома]")
            return
        ...
        # Удаляем дом из магазина
        del HOUSE_SHOP[house_id]
        
        # Сохраняем изменения
        save_house_shop()
        
        bot.send_message(message.chat.id, f"✅ Дом '{house_name}' удален из магазина")
        
    except Exception as e:
        logging.error(f"Ошибка удаления дома: {e}")
        bot.send_message(message.chat.id, f"❌ Ошибка: {str(e)[:100]}")
