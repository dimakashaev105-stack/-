...
def create_main_menu(chat_id):
    ...
    if chat_id > 0:
        markup.add(
            ...
            KeyboardButton("🎭 Рынок ID")  # Добавляем кнопку рынка
        )
    else:
        markup.add(
...
def format_id_card(id_number, user_id=None):
    """Форматирует карточку ID для отображения"""
    if id_number == 1:
        rank = "👑 ЛЕГЕНДА"
        color = "🌟"
    elif id_number <= 3:
    ...
    # Получаем владельца
    owner_id = get_user_by_id_number(id_number)
    owner_name = "Свободен" if not owner_id else "Владелец"
    
    if owner_id:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute('SELECT nickname, username FROM users WHERE user_id = ?', (owner_id,))
        owner_data = cursor.fetchone()
        conn.close()
        
        if owner_data and owner_data[0]:
            owner_name = f"@{owner_data[0]}"
        elif owner_data and owner_data[1]:
            owner_name = f"@{owner_data[1]}"
    
    card = f"""
{color} *ID #{id_number}* {color}
┌────────────────
│ {rank}
│ Владелец: {owner_name}
└────────────────
...
def get_user_id_number(user_id):
    """Получает номер ID пользователя"""
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
...
def is_id_owner(user_id, id_number):
    """Проверяет, является ли пользователь владельцем ID"""
    owner_id = get_user_by_id_number(id_number)
    return owner_id == user_id

# === ГЛАВНОЕ МЕНЮ РЫНКА ===

@bot.message_handler(func=lambda message: message.text.lower() == "🎭 рынок id")
def handle_id_market(message):
    """Главное меню рынка ID"""
    try:
        if is_spam(message.from_user.id):
            return
        ...
        user_id = message.from_user.id
        user_id_number = get_user_id_number(user_id)
        ...
        active_sales = cursor.fetchone()[0]
        
        # Активные предложения для пользователя
        cursor.execute('''
        ...
        markup.add(
            InlineKeyboardButton("🛒 КУПИТЬ ID", callback_data="idmarket_buy"),
            InlineKeyboardButton("💰 ПРОДАТЬ ID", callback_data="idmarket_sell"),
            InlineKeyboardButton("📦 МОИ ЗАПРОСЫ", callback_data="idmarket_my_offers"),
            InlineKeyboardButton(f"📨 ЗАПРОСОВ ({pending_offers})", callback_data="idmarket_pending"),
            InlineKeyboardButton("📊 СТАТИСТИКА", callback_data="idmarket_stats"),
            InlineKeyboardButton("❓ ПОМОЩЬ", callback_data="idmarket_help")
        )
        
        market_text = f"""
🎭 *МИНИМАЛИСТИЧНЫЙ РЫНОК ID*

👤 *Твой ID:* #{user_id_number}
📦 *Продаж:* {active_sales}
📨 *Запросов:* {pending_offers}
...
*Команды:*  
`идкуп (номер)` — купить ID  
`идпрод (сумма)` — выставить на продажу  
`идпред (номер) (сумма)` — предложить цену
        """
        ...
        logging.error(f"Ошибка рынка ID: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка загрузки рынка")
...
def handle_id_buy(message):
    ...
    try:
        ...
        if len(parts) < 2:
            bot.send_message(message.chat.id, 
                           "❌ Формат: `идкуп (номер ID)`\nПример: `идкуп 42`",
                           parse_mode='Markdown')
            return
        
        try:
            target_id_number = int(parts[1])
        except ValueError:
            bot.send_message(message.chat.id, "❌ Номер ID должен быть числом")
            return
        
        # Проверяем, не пытается ли купить свой ID
        user_own_id = get_user_id_number(user_id)
        if user_own_id == target_id_number:
            bot.send_message(message.chat.id, "❌ Нельзя купить свой собственный ID")
            return
        
        # Ищем активную продажу
        conn = get_db_connection()
        cursor = conn.cursor()
        
        cursor.execute('''
            SELECT sale_id, seller_id, price 
            FROM id_sales 
        ...
        if not sale:
            bot.send_message(message.chat.id, 
                           f"❌ ID #{target_id_number} не продается сейчас\n"
                           f"💡 Используй `идпред {target_id_number} (сумма)` чтобы предложить цену",
                           parse_mode='Markdown')
            conn.close()
            return
        ...
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("✅ ПОДТВЕРДИТЬ", callback_data=f"idbuy_confirm_{sale_id}"),
            InlineKeyboardButton("❌ ОТМЕНА", callback_data="idbuy_cancel")
        )
        
        bot.send_message(
            message.chat.id,
            f"🎭 *ПОДТВЕРЖДЕНИЕ ПОКУПКИ*\n\n"
            f"🎯 *ID:* #{target_id_number}\n"
            f"💰 *Цена:* {format_balance(price)}🌸\n"
            f"💳 *Ваш баланс:* {format_balance(balance)}🌸\n\n"
            f"⚠️ После покупки ID станет твоим навсегда!",
            reply_markup=markup,
            parse_mode='Markdown'
        )
        ...
        logging.error(f"Ошибка покупки ID: {e}")
...
def handle_id_sell(message):
    ...
    try:
        ...
        if existing:
            bot.send_message(message.chat.id, 
                           "❌ У тебя уже есть активная продажа!\n"
                           "Сначала отмени её или дождись покупки")
            conn.close()
            return
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?)
        ''', (user_id, user_id_number, price))
        
        conn.commit()
        conn.close()
        
        # Отправляем красивое подтверждение
        card = format_id_card(user_id_number, user_id)
        ...
        bot.send_message(
            ...
            f"{card}\n"
            f"💰 *Цена:* {format_balance(price)}🌸\n\n"
            f"💡 Купить можно командой: `идкуп {user_id_number}`",
            parse_mode='Markdown'
        )
        
    except Exception as e:
        logging.error(f"Ошибка продажи ID: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка при продаже")

# === ПРЕДЛОЖЕНИЕ НА ПОКУПКУ (идпред) ===
...
def handle_id_offer(message):
    ...
    try:
        ...
        # Проверяем, не свой ли ID
        user_own_id = get_user_id_number(user_id)
        if user_own_id == target_id_number:
            bot.send_message(message.chat.id, "❌ Нельзя предложить цену за свой ID")
            return
        
        # Получаем владельца ID
        target_owner_id = get_user_by_id_number(target_id_number)
        
        if not target_owner_id:
            bot.send_message(message.chat.id, 
                           f"❌ ID #{target_id_number} не имеет владельца\n"
                           f"💡 Его можно купить по минимальной цене")
            return
        
        conn = get_db_connection()
        cursor = conn.cursor()
        
        # Проверяем баланс (блокируем сумму)
        balance = get_balance(user_id)
        if balance < price:
            bot.send_message(message.chat.id, 
                           f"❌ Недостаточно средств!\n"
        ...
        cursor.execute('''
            ...
            WHERE buyer_id = ? AND target_id = ? AND status = "pending"
        ''', (user_id, target_owner_id))
        
        existing = cursor.fetchone()
        if existing:
            bot.send_message(message.chat.id, 
                           "❌ У тебя уже есть активное предложение этому пользователю")
            conn.close()
            return
        
        # Блокируем средства (временно)
        update_balance(user_id, -price)
        
        # Создаем предложение
        cursor.execute('''
            INSERT INTO id_offers (buyer_id, target_id, price, status)
            VALUES (?, ?, ?, "pending")
        ''', (user_id, target_owner_id, price))
        ...
        buyer_name = buyer_data[0] or f"@{buyer_data[1]}" if buyer_data[1] else f"ID: {user_id}"
        ...
        markup = InlineKeyboardMarkup(row_width=2)
        markup.add(
            InlineKeyboardButton("✅ ПРИНЯТЬ", callback_data=f"idoffer_accept_{offer_id}"),
            InlineKeyboardButton("❌ ОТКЛОНИТЬ", callback_data=f"idoffer_reject_{offer_id}")
        )
        
        try:
            bot.send_message(
                target_owner_id,
                f"📨 *НОВОЕ ПРЕДЛОЖЕНИЕ О ПОКУПКЕ!*\n\n"
                f"👤 *Покупатель:* {buyer_name}\n"
                f"🎯 *ID:* #{target_id_number}\n"
                f"💰 *Сумма:* {format_balance(price)}🌸\n\n"
                f"💡 Средства покупателя заморожены до твоего решения",
                reply_markup=markup,
                parse_mode='Markdown'
            )
        except:
        ...
        bot.send_message(
            ...
            f"⏳ Ожидай решения продавца",
            parse_mode='Markdown'
        )
        
    except Exception as e:
        logging.error(f"Ошибка создания предложения: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка при создании предложения")

# === ОБРАБОТЧИКИ КНОПОК ===

@bot.callback_query_handler(func=lambda call: call.data.startswith('idbuy_'))
def handle_id_buy_confirm(call):
    """Подтверждение покупки"""
    try:
        user_id = call.from_user.id
        
        if call.data == "idbuy_cancel":
        ...
        if call.data.startswith('idbuy_confirm_'):
            ...
            cursor.execute('''
                SELECT seller_id, id_number, price 
                FROM id_sales 
            ...
            if not sale:
                bot.answer_callback_query(call.id, "❌ Продажа уже неактивна", show_alert=True)
                bot.edit_message_text(
                    "❌ Эта продажа уже недоступна",
                    call.message.chat.id,
                    call.message.message_id
                )
                conn.close()
            ...
            seller_id, id_number, price = sale
            ...
            # Проверяем, не покупает ли свой ID
            if seller_id == user_id:
                bot.answer_callback_query(call.id, "❌ Нельзя купить свой ID", show_alert=True)
                conn.close()
                return
            ...
            cursor.execute('''
                UPDATE id_sales SET status = "sold", buyer_id = ?, sold_at = CURRENT_TIMESTAMP
                WHERE sale_id = ?
            ''', (user_id, sale_id))
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?)
            ''', (seller_id, user_id, id_number, price))
            ...
            # Обновляем сообщение
            card = format_id_card(id_number, user_id)
            ...
            bot.edit_message_text(
                ...
                f"🎉 Поздравляю с приобретением!",
                call.message.chat.id,
                call.message.message_id,
                parse_mode='Markdown'
            )
            
            bot.answer_callback_query(call.id, "✅ ID куплен!")
        ...
        logging.error(f"Ошибка подтверждения покупки: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка", show_alert=True)

@bot.callback_query_handler(func=lambda call: call.data.startswith('idoffer_'))
def handle_id_offer_response(call):
    ...
    try:
        ...
        cursor.execute('''
            SELECT buyer_id, target_id, price, status 
            FROM id_offers 
            WHERE offer_id = ?
        ''', (offer_id,))
        ...
        buyer_id, target_id, price, status = offer
        
        # Проверяем, что отвечает именно владелец ID
        if target_id != user_id:
            bot.answer_callback_query(call.id, "❌ Это не твое предложение", show_alert=True)
            conn.close()
            return
        
        if status != "pending":
            bot.answer_callback_query(call.id, "❌ На это предложение уже ответили", show_alert=True)
            conn.close()
            return
        
        if action == "accept":
            # Получаем номер ID продавца
            seller_id_number = get_user_id_number(user_id)
            
            # Проверяем баланс покупателя (деньги уже заморожены)
            buyer_balance = get_balance(buyer_id)
            
            # Начинаем транзакцию
            cursor.execute('BEGIN IMMEDIATE')
            
            # Переводим деньги продавцу (они уже списаны у покупателя при создании оффера)
            cursor.execute('''
                UPDATE users SET balance = balance + ? WHERE user_id = ?
            ''', (price, user_id))
            ...
            cursor.execute('''
                UPDATE id_offers SET status = "accepted", responded_at = CURRENT_TIMESTAMP
                WHERE offer_id = ?
            ''', (offer_id,))
            ...
            cursor.execute('''
                INSERT INTO id_history (seller_id, buyer_id, id_number, price)
                VALUES (?, ?, ?, ?)
            ''', (user_id, buyer_id, seller_id_number, price))
            ...
            bot.answer_callback_query(call.id, "✅ Предложение принято!")
            ...
            # Обновляем статус оффера
            cursor.execute('''
                UPDATE id_offers SET status = "rejected", responded_at = CURRENT_TIMESTAMP
                WHERE offer_id = ?
            ''', (offer_id,))
        ...
        logging.error(f"Ошибка ответа на предложение: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка", show_alert=True)

# === ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ===

@bot.callback_query_handler(func=lambda call: call.data.startswith('idmarket_'))
def handle_id_market_callbacks(call):
    """Обработка кнопок главного меню"""
    user_id = call.from_user.id
    
    try:
        if call.data == "idmarket_buy":
            # Показать активные продажи
            conn = get_db_connection()
            cursor = conn.cursor()
            
            cursor.execute('''
                SELECT sale_id, seller_id, id_number, price 
                FROM id_sales 
                WHERE status = "active"
                ORDER BY price ASC
                LIMIT 10
            ...
            if not sales:
                bot.answer_callback_query(call.id, "📭 Нет активных продаж")
                return
            
            text = "🛒 *АКТИВНЫЕ ПРОДАЖИ*\n\n"
            
            for sale_id, seller_id, id_number, price in sales:
                card = format_id_card(id_number)
                text += f"{card}\n💰 {format_balance(price)}🌸\n`идкуп {id_number}`\n\n"
            
            bot.send_message(call.message.chat.id, text, parse_mode='Markdown')
            bot.answer_callback_query(call.id)
            
        elif call.data == "idmarket_my_offers":
            # Показать мои предложения
            conn = get_db_connection()
            cursor = conn.cursor()
            
            cursor.execute('''
                SELECT offer_id, target_id, price, status, created_at
                FROM id_offers 
                WHERE buyer_id = ?
            ...
            if not offers:
                bot.answer_callback_query(call.id, "📭 У тебя нет активных предложений")
                return
            
            text = "📦 *МОИ ПРЕДЛОЖЕНИЯ*\n\n"
            
            for offer_id, target_id, price, status, created_at in offers:
                target_id_number = get_user_id_number(target_id)
                ...
                text += f"{status_emoji} ID #{target_id_number} — {format_balance(price)}🌸\n"
            
            bot.send_message(call.message.chat.id, text, parse_mode='Markdown')
            bot.answer_callback_query(call.id)
            
        elif call.data == "idmarket_pending":
            # Показать входящие предложения
            conn = get_db_connection()
            cursor = conn.cursor()
            
            cursor.execute('''
                SELECT offer_id, buyer_id, price, created_at
                FROM id_offers 
                WHERE target_id = ? AND status = "pending"
                ORDER BY created_at DESC
            ''', (user_id,))
            ...
            if not offers:
                bot.answer_callback_query(call.id, "📭 Нет входящих предложений")
                return
            
            for offer_id, buyer_id, price, created_at in offers:
                buyer_id_number = get_user_id_number(buyer_id)
                ...
                bot.send_message(
                    call.message.chat.id,
                    f"📨 *ПРЕДЛОЖЕНИЕ*\n\n"
                    f"👤 *Покупатель:* ID #{buyer_id_number}\n"
                    f"💰 *Сумма:* {format_balance(price)}🌸",
                    reply_markup=markup,
                    parse_mode='Markdown'
                )
            ...
            cursor.execute('SELECT COUNT(*) FROM id_offers WHERE status = "pending"')
            pending_offers = cursor.fetchone()[0]
            
            cursor.execute('SELECT COUNT(*) FROM id_history')
            total_trades = cursor.fetchone()[0]
            
            cursor.execute('SELECT SUM(price) FROM id_history')
            total_volume = cursor.fetchone()[0] or 0
            
            cursor.execute('''
                SELECT price FROM id_history 
                ORDER BY sold_at DESC LIMIT 1
            ''')
            last_trade = cursor.fetchone()
            last_price = last_trade[0] if last_trade else 0
...
📦 *Активных продаж:* {active_sales}
📨 *Активных офферов:* {pending_offers}
🔄 *Всего сделок:* {total_trades}
💰 *Общий объем:* {format_balance(total_volume)}🌸
💎 *Последняя сделка:* {format_balance(last_price)}🌸
            """
            
            bot.send_message(call.message.chat.id, stats_text, parse_mode='Markdown')
            bot.answer_callback_query(call.id)
            
        elif call.data == "idmarket_help":
            help_text = """
🎭 *РЫНОК ID — ПОМОЩЬ*

*КОМАНДЫ:*
┌────────────────
│ `идкуп (номер)` — купить ID с рынка
│ `идпрод (сумма)` — продать свой ID
│ `идпред (номер) (сумма)` — предложить цену
└────────────────
...
🔹 *Покупка с рынка*
Если ID выставлен на продажу — покупаешь сразу

🔹 *Продажа своего ID*
Выставляешь цену и ждешь покупателя

🔹 *Предложение цены*
Если ID не продается — предлагаешь свою цену
Средства замораживаются до решения продавца
...
💡 *Чем меньше номер — тем ценнее ID!*
            """
        ...
        logging.error(f"Ошибка в меню рынка: {e}")
        bot.answer_callback_query(call.id, "❌ Ошибка")

# === ОТМЕНА ПРОДАЖИ (для владельца) ===

@bot.message_handler(func=lambda message: message.text.lower().startswith('идоtmена'))
def handle_id_cancel_sale(message):
    """Отмена продажи своего ID"""
    try:
        if is_spam(message.from_user.id):
            return
        ...
        logging.error(f"Ошибка отмены продажи: {e}")
